...
@Composable
fun ProfileTopBar(navHostController: NavHostController, vm: ProfileViewModel = hiltViewModel()) {
    val activity = LocalContext.current as? Activity
    val idUser = vm.user?.id
    TopAppBar(title = { Text(text = stringResource(R.string.profile)) },
        ...
        actions = {
            ...
            DropdownMenu(expanded = vm.showMenu, onDismissRequest = { vm.showMenu = false }) {
                DropdownMenuItem(onClick = {
                    vm.user?.let { user ->
                        navHostController.navigate("${Graph.PROFILE}/${user.toJson()}")
                    }
