...
class ResultingActivityHandler {
    ...
    suspend fun takePicturePreview(maxTry: Int = 10, millis: Long = 200): Bitmap? {
        return request(ActivityResultContracts.TakePicturePreview(), maxTry, millis) { it.launch() }
    }

    suspend fun getContent(type: String, maxTry: Int = 10, millis: Long = 200): Uri? {
        return request(ActivityResultContracts.GetContent(), maxTry, millis) { it.launch(type) }
    }
